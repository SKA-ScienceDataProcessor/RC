{-
New AST for low-level DNA. With it program could be analyzed without
running it. This way we know all actor we want to launch and can
schedule them with at least small dose of intelligence

Only missing part at the moment is condition operator. It's quite
...
type DNA  = Program DnaF
type Kern = Program KernF

-- We do not pass values themselves inside DSL. Instead we pass opaque
-- boxes so we can reify program.
data Val a where
  Val :: Typeable a => Int -> Val a

data ValSet a where
  ValSet :: Typeable a => Int -> ValSet a

data Res      = Res
data ResGroup = ResGroup

data Shell a b where
  Shell :: (Typeable a, Typeable b) => Int -> Shell a b

data CollectActor a b = CollectActor

-- Commands for kernel.
--
-- Monad for executing kernels is operational monad too so we can find
-- out which values produced by actors/kernels are used here.
data KernF a where
  Use :: Val a -> KernF a
  IO  :: IO a  -> KernF a

-- Commands for DNA program
data DnaF a where
