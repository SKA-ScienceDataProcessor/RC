...
clean:
	...
	LG_RT_DIR=$(SDP_BUILDDIR)/Legion-udp/runtime GASNET=$(SDP_BUILDDIR)/gasnet-udp/release SHARED_LOWLEVEL=0 USE_GASNET=1 CONDUIT=udp make -f $(SDP_BUILDDIR)/Legion-udp/runtime/runtime.mk clean
ifeq ($(SDP_USE_IBV),1)
	rm -f $(SDP_BUILDDIR)/Legion-ibv/bindings/terra/*.o $(SDP_BUILDDIR)/Legion-ibv/bindings/terra/*.so
	LG_RT_DIR=$(SDP_BUILDDIR)/Legion-ibv/runtime GASNET=$(SDP_BUILDDIR)/gasnet-ibv/release SHARED_LOWLEVEL=0 USE_GASNET=1 CONDUIT=ibv make -f $(SDP_BUILDDIR)/Legion-ibv/runtime/runtime.mk clean
endif
...
$(SDP_BUILDDIR)/Legion-ibv/bindings/terra/liblegion_terra.so:
	CONDUIT=ibv GASNET_CONDUIT=ibv GASNET=$(SDP_BUILDDIR)/gasnet-ibv/release OUTPUT_LEVEL=$(OUTPUT_LEVEL) CC_FLAGS="$(CC_FLAGS)" $(SDP_BUILDDIR)/Legion-ibv/language/install.py --with-terra=$(SDP_BUILDDIR)/terra/release --gasnet || exit 1
...
$(SDP_BUILDDIR)/Legion-udp/bindings/terra/liblegion_terra.so:
	CONDUIT=udp GASNET_CONDUIT=udp GASNET=$(SDP_BUILDDIR)/gasnet-udp/release OUTPUT_LEVEL=$(OUTPUT_LEVEL) CC_FLAGS="$(CC_FLAGS)" $(SDP_BUILDDIR)/Legion-udp/language/install.py --with-terra=$(SDP_BUILDDIR)/terra/release --gasnet || exit 1
$(LIBRARY)-local.so: $(LIBRARY_SRCS) $(SDP_BUILDDIR)/Legion-udp/bindings/terra/liblegion_terra.so
	...
	CONDUIT=udp GASNET_CONDUIT=udp LG_RT_DIR=$(SDP_BUILDDIR)/Legion-udp/runtime OUTFILE=$(LIBRARY)-local.so \
