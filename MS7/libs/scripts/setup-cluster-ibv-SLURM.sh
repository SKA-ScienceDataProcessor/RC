...
clonepull https://github.com/StanfordLegion/legion Legion-udp-noprof

# Go to Regent place.
cd Legion-udp-noprof
git checkout master
cd language
...
# now with profiling.
cd $BUILDDIR
clonepull Legion-udp-noprof Legion-udp-prof
cd Legion-udp-prof/language
CONDUIT=udp OUTPUT_LEVEL="LEVEL_DEBUG" ./install.py --with-terra=$TERRA_DIR --gasnet || exit 1
...
if [ "$with_ibv" == "1" ] ; then
  ...
  export GASNET="$GASNET_ROOT"
  export GASNET_BIN="$GASNET_ROOT/bin"
  clonepull Legion-udp-noprof Legion-ibv-noprof

  # Go to Regent place.
  cd Legion-ibv-noprof/language
  ...
  # Repeat with profiling.
  clonepull Legion-udp-noprof Legion-ibv-prof

  # Go to Regent place.
  cd Legion-ibv-prof/language

  # Running the installation, enabling the GASnet.
  OUTPUT_LEVEL=LEVEL_DEBUG CONDUIT=ibv ./install.py --with-terra=$TERRA_DIR --gasnet || exit 1
