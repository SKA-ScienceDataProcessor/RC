...
$PWD/dna_common.sh dna_check_rc $slurmJobNodeList $JOBID $rc
if [ $? -eq 0 ]; then
	echo -e "dna_check_rc performed!!"
else
	echo -e "Some issue occured in dna_check_rc !!"
	exit
fi

$PWD/dna_common.sh generatingCADFile $slurmJobNodeList $JOBID ""
if [ $? -eq 0 ]; then
...
$PWD/dna_common.sh startingCHprocess $slurmJobNodeList $JOBID $rc
if [ $? -eq 0 ]; then
