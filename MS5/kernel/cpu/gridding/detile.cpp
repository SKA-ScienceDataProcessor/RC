...
int main(int argc, char **argv) {
    ...
    const int _CPLX_FIELDS = 2;

    ImageParam in_grid(type_of<double>(), 3, "in_grid");
    in_grid.set_extent(0, _CPLX_FIELDS).set_stride(0,1).set_stride(1, _CPLX_FIELDS);

    Func out_grid("out_grid"); Var cmplx("c"),x("x"),y("y");
    out_grid(cmplx,x,y) = undef<double>();

    // Determine bounds. As tiles with margin are expected to go
    // outside the field, we have to account for *both* buffer bounds
    // here.
    ...
    out_grid.output_buffer().set_extent(0, _CPLX_FIELDS).set_stride(0,1).set_stride(1, _CPLX_FIELDS);
    out_grid.bound(cmplx, 0, _CPLX_FIELDS);
    out_grid.update().vectorize(rc, 2);
