...
foreign import ccall "binm" cbinm :: CString -> CInt -> CInt -> CDouble -> Ptr (Complex Double) -> Ptr CDouble -> IO Int
...
bin prefix isHalfGCF td = throwIf_ (/= 0) (printf "Error %d while binning...") mkbin
  where
    ...
    mkbin = withCString prefix $ \cpre -> binner cpre (fi $ tdChannels td) (fi $ tdPoints td) scale (tdVisibilies td) (tdUVWs td)
